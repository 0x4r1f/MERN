class myClass {
    // properties 
    subject = "MERN";
    students = ["Morshed", "khorsed", "siam", "sagor", "pintu"];
    firstStudent = this.students[0];
    siam;

    // methods 
    sum = (num1 , num2) => {
        if(isNaN(num1) || isNaN(num2)){
            return "Invalid Number";
        }
        // return parseFloat(num1) + parseFloat(num2);
        // return parseInt(num1) + parseInt(num2);
        return Number(num1) + Number(num2);
    }

    // constructor 
    constructor() {
        console.log("Class Initialized");
    }

}

const obj = new myClass ("Parent class Initialized");
const otherIns = new myClass;
otherIns.siam = "New valu provided";
otherIns.sagor = "Made in mizmizi";
otherIns.morshed = (data) => data;
console.log(otherIns.siam);
console.log(otherIns.sagor);
console.log(otherIns.morshed("Khorshed Morshed ar classmate"));

console.log(obj.subject);
console.log(obj.students[3]);
console.log(obj.sum("15", "10")); 

// inheritance 
class myChildClass extends myClass {
    myBestStudent = () => {
        return this.students[4];
    }
}

const obj2 = new myChildClass("Child Class Initialized");
console.log(obj2.myBestStudent());